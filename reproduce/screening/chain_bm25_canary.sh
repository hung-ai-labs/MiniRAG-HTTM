...
# Chạy lại được: tầng A đã có báo cáo, V3 đã FROZEN, canary đã có quyết định thì dùng lại — không rút lại, không ghi đè.
cd "$(dirname "$0")/../.." || exit 1
...
decision () { "$PY" -c "import json,sys; print(json.load(open(sys.argv[1])).get('decision') or '')" "$1" 2>/dev/null; }
...
for v in b1 b2; do
  ...
  elif [ -n "$(decision logs/screening/$v/canary_report.json)" ]; then
    stage "CANARY_$v (đã có quyết định — dùng lại)"
  else
    stage "CANARY_$v"
